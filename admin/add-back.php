<?php 
	...
	$target_file = "../images/" . basename($_FILES["FileImage"]["name"]);
    $uploadOk = 1;
...
<?php
        ...
        // truy vấn
        $sql = "INSERT INTO products(name, category_id, img, description, price, date_added, quantity, status) 
                VALUES('$namePr', '$categoryPr', '$image', '$descriptPr', '$pricePr', now(), '$quantityPr', '$status');";
        $result = mysqli_query($conn,$sql);
        if($result)
        {
            header("location:add.php?addps=success");
            exit();
        }
        else {
            header("location:add.php?addpf=fail");
            exit();
        }
    }
 ?>